const opSymbols = new Set(['+', '-', '*', '/', '&', '|', '<', '>', '=']);const unarySymbols = new Set(['-', '~']);
...
function parseClass(tokens, tokenIndex) {
    ...
    children.push(tokens[tokenIndex++]); // 'class'
    children.push(tokens[tokenIndex++]); // className
...
function parseLetStatement(tokens, tokenIndex) {
    ...
    children.push(tokens[tokenIndex++]); // var name
